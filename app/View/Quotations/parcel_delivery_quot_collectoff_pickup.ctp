...
function validatefields(val)
{
	...
	if(document.getElementById('AddressFirstAddress').value==''){
		...
		document.getElementById('AddressFirstAddress').focus();
		return false;
	}
...
<div id="main_wapper">
	 <h1><?php echo $this->Html->link('Back to Compare Price',array('Controller'=>'quotations','action'=>'quotation_compare_price',T)); ?></h1>
	  <?php echo $this->Form->create('Address',array('url'=>array('controller'=>'quotations','action'=>'parcel_delivery_quot_collectoff_pickup'),'onsubmit'=>'return validatefields();'))?>
			...
			function getWeekDays(){
				 ...
				 $days_arr=Array();
 				 $j=0;
	    			...
	    			<?php echo "<input type='radio' name='data[Address][day]' id='day' value='".$day_val."'/><label>$day<lable>";?>
		<?php  $j=$j+1;
		}		
    }
}
...
?>
    ...
    <div class="sender_form">
	 ...
	 <?php echo $this->Form->hidden('Lineup.id',array('value'=>$lineup_id));?>
      ...
      <div class="sender_row">
        <label>Country :</label>
        <?php echo $this->Form->text('source_country',array('value'=>'UK','readonly' => 'readonly'));?>
      </div>
      <div class="sender_row">
        <label>Telephone :</label>
