...
<?php if($compansion){?>		
	...
	<div class="summary">
		...
		<?php }?>							
	<div class="summary-row submit">	<?php echo $this->Html->link('Continue',array('controller'=>'franchises','action'=>'admin_franchies_payment'));?></div>
	</div>
			<?php #foreach($compansion as $comp){} ?>
			<td>
			</td><td> 
...
<?php }else{?>
	...
	<div class="sender_form">					
			 <?php echo $this->Form->create('Parcel',array('url'=>array('controller'=>'franchises','action'=>'admin_franchises_parcel_compansation'),'onSubmit'=>'return validateParcelDetail()'))?>
